...
if(isset($_SESSION['username'])){
  $stmtb = $pdo->prepare('SELECT * FROM mentor');
  $stmtb->execute(array(
...
if(isset($_POST['submit'])){
  $stmt = $pdo->prepare('INSERT INTO domain_mentor (mentor_id, domain_id) VALUES (:mentor_id, :domain_id)');

  $stmt->execute(array(
    ':mentor_id' => (int)$_POST['field1'],
    ':domain_id' => (int)$_POST['field2']
  )
  );
  ...
  <div class="flex text-gray-800 min-h-screen">
    ...
    <div class="flex-grow bg-gray-100">
      ...
      <main class="flex flex-col space-y-10 px-6 py-5 max-w-6xl mx-auto">
       ...
       <header class="text-xl font-semibold" style="Text-align:center; font-size:40px; ">Assign Domain to Teachers</header>
        <div class="form-style-5" id="submit">

          <form method="POST">
            <fieldset>
              <legend><span class="number">1</span> Domain Details</legend>

              <div class="dropdown">
                <span>&nbsp;&nbsp;&nbsp;Teacher</span>
                <select id="branch" name="field1">
                ...
                <span>&nbsp;&nbsp;&nbsp;Domain</span>
                <select id="branch" name="field2">
                   <optgroup label="Teachers">
                  <option value=""><b>Select Domain</b></option>
                  <?php $i=1;
                       $stmtc = $pdo->query('SELECT * FROM domain');
                       $rowc = $stmtc->fetchAll(PDO::FETCH_ASSOC);
                       foreach ($rowc as $rowsc) {
                         $rcb=$rowsc['domain_name'];
                         echo
                         '<option value='.$rowsc['domain_id'].'>'.$rcb.'</option>';
                         $i++;
                       }
                  ?>
                </optgroup>
